...
void main()
{
    ...
    if (ch=='y'||ch=='Y')
    {
        f1=fopen("begining.txt","r");https://www.onlinegdb.com/edit/Q-k2Nd8Pto#tab-stdin
      while(feof(f1)==0)
      {
          s=getc(f1);
          printf("%c",s);
    ...
    else
    {
        ending();
            ...
            else if((ch3=='y'||ch3=='Y')&&(ch2!='y'||ch2!='Y'))
            {
                printf("magnus try to fight the wolves off with his fist but the beasts were fast and came at him from all direction attacking his legs and biting off his hands and finally killed him by biting off his head\n\t\t\t\tGAME OVER ");
            }
            else
               printf("mangus,you have chose not to fight the monsters.magnus ran away but the wolves were much faster\n they leaped on magnus in a pack and killed him\n\t\t\t\tGAME OVER");
        }
        else
        printf("Magnus ran away in fear and failed to avenge his parents.\nAs Magnus failed keep his promise spirits from the underworld dragged him to hell\n\t\t\t\tGAME OVER ");
